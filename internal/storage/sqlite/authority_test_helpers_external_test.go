...
func claimEffectiveControlsWorkspaceAuthority(t *testing.T, ctx context.Context, store *sqlite.Store, workspaceID string) sqlite.WorkspaceAuthorityRecord {
	t.Helper()
...
`, node.AuthorityNodeID, node.NodeKind, node.HostLabel, node.BootInstanceID, registeredAt, referenceAt, string(sqlite.RuntimeNodeStatusOnline)); err != nil {
	...
	if existing, err := store.GetWorkspaceAuthority(ctx, workspaceID, "workspace"); err == nil {
		...
		if leaseToken == "" || existing.HolderAuthorityNodeID != node.AuthorityNodeID || existing.Status != sqlite.WorkspaceAuthorityStatusActive {
			leaseToken = "lease-sqlite-test-" + workspaceID
		}
		if _, err := store.DB().ExecContext(ctx, `
...
ON CONFLICT(workspace_id, scope) DO UPDATE SET
	...
	status = excluded.status,
	updated_at = excluded.updated_at
`, workspaceID, "workspace", node.AuthorityNodeID, "lease-sqlite-test-"+workspaceID, int64(1), now.Add(time.Hour).Format(time.RFC3339Nano), int64(1), int64(1), string(sqlite.WorkspaceAuthorityStatusActive), referenceAt); err != nil {
		t.Fatalf("seed workspace authority for %s: %v", workspaceID, err)
	}
	record, err := store.GetWorkspaceAuthority(ctx, workspaceID, "workspace")
	if err != nil {
...
func claimExternalWorkspaceAuthority(t *testing.T, ctx context.Context, store *sqlite.Store, workspaceID string) sqlite.WorkspaceAuthorityRecord {
	t.Helper()
	return claimEffectiveControlsWorkspaceAuthority(t, ctx, store, workspaceID)
}

func claimExternalTaskForSessionStart(t *testing.T, ctx context.Context, store *sqlite.Store, workspaceID, taskID, agentID string) {
	t.Helper()
